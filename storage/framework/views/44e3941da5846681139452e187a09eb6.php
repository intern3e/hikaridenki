...
<head>
  ...
  <title><?php echo e($product->name ?? 'รายละเอียดสินค้า'); ?> | PowerCare by Hikari</title>
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 pt-3 pb-4 md:pb-6">
      <nav aria-label="Breadcrumb" class="text-xs md:text-sm mb-1.5">
        <ol class="flex items-center gap-2">
          <li><a href="<?php echo e(url('/')); ?>" class="text-[color:var(--brand)] hover:underline">หน้าแรก</a></li>
          <li class="text-slate-400">/</li>
          <li><a href="<?php echo e(route('showproduct')); ?>" class="text-[color:var(--brand)] hover:underline">ประเภทสินค้า</a></li>
          <li class="text-slate-400">/</li>
          <li class="font-medium line-clamp-1 text-[color:var(--brand)]"><?php echo e($modelTxt ?: $nameTxt); ?></li>
        </ol>
      </nav>

      <h1 class="titlebar mt-2" aria-label="<?php echo e(trim($brandTxt.' รุ่น • '.($moTxt ?: $modelTxt ?: ''))); ?>">
        <span class="brand"><?php echo e($brandTxt ?: '—'); ?></span>
        <?php if($moTxt || $modelTxt): ?>
          <span class="model">รุ่น • <span class="tabnums"><?php echo e($moTxt ?: $modelTxt); ?></span></span>
        <?php endif; ?>
      </h1>

      
      <?php if($hasCore || $hasName): ?>
        <p class="subline mt-1">

          <?php if($hasName): ?>
            <span class="subline-name"><?php echo e($nameClean); ?></span>
          <?php endif; ?>
          <?php if($skuTxt): ?>
            <span> • SKU: <?php echo e($skuTxt); ?></span>
          <?php endif; ?>
        </p>
      <?php endif; ?>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
      ...
      <aside class="lg:col-span-5">
        <div class="sticky-buy soft rounded-2xl bg-white p-5 md:p-6 space-y-5">
          ...
          <?php if($ratingAvg > 0): ?>
            <div class="flex items-center gap-2 text-slate-700" aria-label="คะแนน <?php echo e(number_format($ratingAvg,1)); ?> จาก <?php echo e($ratingCount); ?> รีวิว">
              <?php
              ...
              <span class="text-sm">(<?php echo e($ratingCount); ?>)</span>
            </div>
          <?php endif; ?>

          
          <?php if(count($colorList)): ?>
            <div>
              <div class="font-medium mb-2">เลือกสี</div>
              <div class="flex flex-wrap gap-2">
                <?php $__currentLoopData = $colorList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <button type="button" class="px-3 py-1.5 rounded-full border border-slate-300 hover:bg-slate-50 text-sm" aria-pressed="false"><?php echo e($c); ?></button>
          ...
          <div class="rounded-xl border border-slate-200 p-4 space-y-3">
            <div class="flex items-center gap-3">
              ...
              <div class="text-sm">
                ...
                <div class="text-slate-600">พร้อมจัดส่ง: <strong><?php echo e($leadTxt); ?></strong></div>
              </div>
            </div>
            <div class="flex items-center gap-3">
              <i class="bi bi-box-seam <?php if($inStock===false): ?> text-rose-600 <?php else: ?> text-emerald-600 <?php endif; ?> text-xl"></i>
              <div class="text-sm">
                <div class="font-semibold">สต็อก</div>
                <div class="text-slate-600">
                  <strong><?php echo e($stockTxt); ?></strong>
                  <?php if(!is_null($inStock)): ?>
                    <span class="<?php if($inStock): ?> text-emerald-600 <?php else: ?> text-rose-600 <?php endif; ?>">(<?php if($inStock): ?> มีของ <?php else: ?> สินค้าหมด <?php endif; ?>)</span>
                  <?php endif; ?>
                </div>
              </div>
            </div>
