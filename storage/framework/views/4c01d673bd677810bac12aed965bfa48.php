  ...
  <header class="sticky top-0 z-50 transition-all bg-white shadow" role="banner">
    <div class="max-w-7xl mx-auto flex items-center justify-between py-3.5 px-4 md:px-6">
      <a href="/" class="flex items-center gap-2">
        <img src="<?php echo e(asset('storage/logo/PNG.png')); ?>" alt="PowerCare logo" class="w-8 h-8 object-contain" loading="eager" decoding="async" fetchpriority="high">
    ...
    <div class="grid grid-cols-1 md:grid-cols-[260px_minmax(0,1fr)] gap-5">
      <aside class="md:sticky md:top-20 self-start">
        <div class="bg-white border rounded-xl soft">
          ...
          <?php $currentBrandSlug = $activeBrandSlug ?? '*'; ?>

          <div class="hidden md:block max-h-[70vh] overflow-y-auto no-scrollbar py-2">
            <a href="<?php echo e(route('showproduct')); ?>"
               class="block px-3 py-2 text-sm <?php echo e($currentBrandSlug==='*' ? 'bg-blue-600 text-white rounded-md mx-2' : 'hover:bg-slate-50'); ?>">ทั้งหมด</a>
            <?php $__currentLoopData = $brandCounts->keys(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $brandName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <?php $slug = \Illuminate\Support\Str::slug($brandName, '-'); $isActive = $currentBrandSlug === $slug; ?>
              <a href="<?php echo e(route('showproduct.bybrand', ['brand' => $brandName])); ?>"
                 class="block px-3 py-2 text-sm mx-2 rounded-md <?php echo e($isActive ? 'bg-blue-600 text-white' : 'hover:bg-slate-50'); ?>"><?php echo e($brandName); ?></a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </div>

          <div class="md:hidden p-3">
            <select class="w-full border rounded-lg px-3 py-2 text-sm" onchange="if(this.value) location.href=this.value;">
              <option value="<?php echo e(route('showproduct')); ?>" <?php echo e($currentBrandSlug==='*' ? 'selected' : ''); ?>>ทั้งหมด</option>
              <?php $__currentLoopData = $brandCounts->keys(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $brandName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php $slug = \Illuminate\Support\Str::slug($brandName, '-'); $url  = route('showproduct.bybrand', ['brand' => $brandName]); ?>
                <option value="<?php echo e($url); ?>" <?php echo e($currentBrandSlug===$slug ? 'selected' : ''); ?>><?php echo e($brandName); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
          </div>
        </div>
  ...
  <?php else: ?>
    <div class="grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="relative">
          <img
            src="<?php echo e($item->pic_resolved ?? ''); ?>"
          ...
          <div class="p-3 sm:p-4 flex-1 flex flex-col">
            ...
            <h3 class="font-semibold text-sm sm:text-base mb-2 line-2">
              <?php echo e($item->name ?? $item->model ?? '—'); ?>

            </h3>

            
            <div class="text-lg sm:text-xl font-bold text-amber-600 mb-3">
              <?php echo e($item->webpriceTHB); ?> ฿
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm mb-3">
              ...
              <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full <?php echo e($leadChip); ?>">
                <i class="bi bi-truck"></i>
                ส่ง: <strong><?php echo e($leadTxt); ?></strong>
              </span>

              <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full <?php echo e($stockChip); ?>">
                <i class="bi bi-box-seam"></i>
                คงเหลือ: <strong><?php echo e($stockTxt); ?></strong>
              </span>
            </div>

            
            <a href="<?php echo e(route('showproduct.byiditem', ['iditem' => $item->iditem])); ?>"
